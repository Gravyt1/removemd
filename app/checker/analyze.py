...
def check_image(file, fmt):
    try:
        img = Image.open(file)
        meta = {}
        if img.info:
            meta.update({k: str(v) for k, v in img.info.items()})
        ...
        return {"error": "Error checking image metadata"}
...
def check_pdf(file):
    try:
        ...
        return {"error": "Error checking PDF metadata"}
...
def check_office(file, fmt):
    try:
        ...
        if fmt == "DOCX":
            ...
            return {"error": f"Unsupported Office format {fmt}"}
        ...
        return {"error": "Error checking Office metadata"}

def check_audio(file, fmt):
    try:
        file.seek(0)
        ...
        return {"error": "Error checking audio metadata"}
...
def check_video(file, fmt):
    try:
        ...
        if result.returncode == 0:
            ...
            return {"error": "Error extracting video metadata"}
    except Exception as e:
        return {"error": f"Error checking video metadata: {str(e)}"}
